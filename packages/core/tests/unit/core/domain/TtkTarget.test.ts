...
import { TtkTargetFakeBuilder } from '../../../fakes';

describe('TtkTarget', () => {
  describe('constructor', () => {
    it('should create valid target with turns and actions', () => {
      const target = new TtkTargetFakeBuilder().build();

      expect(target.turns).toBe(3);
      expect(target.actions).toBe(8);
      expect(target.tolerancePercent).toBe(15); // Default
    });

    it('should accept custom tolerance', () => {
      const target = new TtkTargetFakeBuilder().withTolerance(20).build();

      expect(target.tolerancePercent).toBe(20);
    });

    it('should accept 0% tolerance', () => {
      const target = new TtkTargetFakeBuilder().withZeroTolerance().build();

      expect(target.tolerancePercent).toBe(0);
    });

    it('should accept 100% tolerance', () => {
      const target = new TtkTargetFakeBuilder().withFullTolerance().build();

      expect(target.tolerancePercent).toBe(100);
    });

    it('should throw ValidationError if turns < 1', () => {
      const builder = new TtkTargetFakeBuilder().withInvalidTurns();

      expect(() => builder.build()).toThrow(ValidationError);
      expect(() => builder.build()).toThrow('TTK turns must be >= 1');
    });

    it('should throw ValidationError if actions < 1', () => {
      const builder = new TtkTargetFakeBuilder().withInvalidActions();

      expect(() => builder.build()).toThrow(ValidationError);
      expect(() => builder.build()).toThrow('TTK actions must be >= 1');
    });

    it('should throw ValidationError if tolerance < 0', () => {
      const builder = new TtkTargetFakeBuilder().withNegativeTolerance();
    ...
    it('should throw ValidationError if tolerance > 100', () => {
      const builder = new TtkTargetFakeBuilder().withAboveMaxTolerance();
  ...
  describe('toleranceFraction', () => {
    ...
    it('should convert 50% to 0.5', () => {
      const target = new TtkTargetFakeBuilder().withTarget(10, 40).withTolerance(50).build();
  ...
  describe('turnsRange', () => {
    ...
    it('should never go below 1', () => {
      const target = new TtkTargetFakeBuilder().withTarget(1, 10).withTolerance(50).build();
      const range = target.turnsRange;
    ...
    it('should floor min and ceil max', () => {
      const target = new TtkTargetFakeBuilder().withTarget(10, 40).withTolerance(20).build();
      const range = target.turnsRange;
  ...
  describe('actionsRange', () => {
    ...
    it('should never go below 1', () => {
      const target = new TtkTargetFakeBuilder().withTarget(1, 1).withFullTolerance().build();
      const range = target.actionsRange;
    ...
    it('should floor min and ceil max', () => {
      const target = new TtkTargetFakeBuilder().withTarget(10, 50).withTolerance(10).build();
      const range = target.actionsRange;
  ...
  describe('equals', () => {
    it('should return true for identical targets', () => {
      const target1 = new TtkTargetFakeBuilder().withTarget(10, 40).withTolerance(15).build();
      const target2 = new TtkTargetFakeBuilder().withTarget(10, 40).withTolerance(15).build();
    ...
    it('should return false for different turns', () => {
      const target1 = new TtkTargetFakeBuilder().withTarget(10, 40).withTolerance(15).build();
      const target2 = new TtkTargetFakeBuilder().withTarget(11, 40).withTolerance(15).build();
    ...
    it('should return false for different actions', () => {
      const target1 = new TtkTargetFakeBuilder().withTarget(10, 40).withTolerance(15).build();
      const target2 = new TtkTargetFakeBuilder().withTarget(10, 41).withTolerance(15).build();
    ...
    it('should return false for different tolerance', () => {
      const target1 = new TtkTargetFakeBuilder().withTarget(10, 40).withTolerance(15).build();
      const target2 = new TtkTargetFakeBuilder().withTarget(10, 40).withTolerance(20).build();
  ...
  describe('toString', () => {
    ...
    it('should format with single-digit values', () => {
      const target = new TtkTargetFakeBuilder().withTarget(5, 20).withTolerance(10).build();
